...
func TestModel_Update_KeyEnter_WithInput(t *testing.T) {
	...
	case e := <-eventsCh:
		...
		ume, ok := e.(junk.UserMessageEvent)
		require.True(t, ok)
...
func TestModel_Update_Turn_Assistant_DoesNotResetExpandAllDetails(t *testing.T) {
	...
	newM, _ := m.Update(turnMsg{turn: turn})
	mm := newM.(*model)
	assert.True(t, mm.expandAllDetails, "new assistant turn should not reset expandAllDetails")
}
...
func TestModel_Update_AutoScrollLock_PreservesBottom(t *testing.T) {
	...
	t.Run("KeyPressMsg Enter with input", func(t *testing.T) {
		m := mkModel()
		m.eventsCh = make(chan junk.Event, 10)
		m.textarea.SetValue("hello")
		newM, _ := m.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
		mm := newM.(*model)
