...
// eventKindSchemaNames returns the mapping of event kind strings to their
// corresponding schema names in the OpenAPI spec.
func eventKindSchemaNames() map[string]string {
...
// defaultSubscriptionKinds returns the handler's default event kinds.
// This is kept in sync with the literal values in handler.go.
func defaultSubscriptionKinds() []string {
	return []string{
		"text",
		"reasoning",
		"tool_call",
...
// extractEventKindEnum extracts the EventKind enum values from the OpenAPI spec.
func extractEventKindEnum(t *testing.T) []string {
	t.Helper()

	spec := loadOpenAPISpec(t)
	schemaRef, ok := spec.Components.Schemas["EventKind"]
	if !ok {
		t.Fatalf("EventKind schema not found in openapi.yaml")
	}

	schema := schemaRef.Value
	if schema == nil {
		t.Fatalf("EventKind schema has nil value")
	}
	...
	for _, v := range schema.Enum {
		s, ok := v.(string)
		if !ok {
			t.Fatalf("EventKind enum contains non-string value: %v", v)
		}
