...
// slice indicates total tokens exceed a safety margin of the
// compactor's Spec.Window. The safety margin is configurable via the
// Ratio field; a zero Ratio is treated as the default (0.8), so
// existing callers get the corrected behavior without code changes.
// Callers that need raw-window behavior (fire at the exact window
// boundary) set Ratio: 1.0 explicitly.
type TokenUsageTrigger struct {
	Spec models.Spec
	// Ratio is the fraction of Spec.Window at which compaction
	// fires. A zero value is treated as 0.8: a safety margin so
	// compaction triggers before the upstream truncates the
	// conversation. Set Ratio: 1.0 to fire at the exact window
	// boundary. The threshold is computed as
	// int(float64(t.Spec.Window) * t.Ratio); the comparison is
	// TotalTokens >= threshold.
	Ratio float64
}

// defaultTokenUsageRatio is the safety margin applied when
// TokenUsageTrigger.Ratio is the zero value. The default is
// 80% of Spec.Window: enough headroom to summarize the
// conversation before the upstream truncates it, but not so
// aggressive that trivial short conversations trigger a
// compaction pass. A non-zero Ratio overrides this default
// entirely.
const defaultTokenUsageRatio = 0.8

// ShouldCompact scans the turn slice from the end for the most recent
// artifact.Usage. If found and Usage.TotalTokens meets or exceeds
// the configured threshold (Spec.Window * Ratio, with the default
// Ratio of 0.8), it returns true. If no Usage artifact is present,
// or Spec.Window is zero, it returns false (graceful degradation).
func (t TokenUsageTrigger) ShouldCompact(turns []state.Turn) bool {
	...
	ratio := t.Ratio
	if ratio == 0 {
		ratio = defaultTokenUsageRatio
	}
	// A non-positive ratio is a configuration error: fall back to
	// the default rather than divide-by-zero or compare against a
	// negative threshold. A negative ratio would never fire
	// (positive usage can never be strictly less than a negative
	// threshold).
	if ratio <= 0 {
		ratio = defaultTokenUsageRatio
	}
	threshold := int(float64(t.Spec.Window) * ratio)
	for i := len(turns) - 1; i >= 0; i-- {
		for _, art := range turns[i].Artifacts {
			if u, ok := art.(artifact.Usage); ok {
				return u.TotalTokens >= threshold
			}
		}
	}
	return false
