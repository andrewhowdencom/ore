...
func TestHandler_BudgetWithinLimit(t *testing.T) {
	r := toolpkg.NewRegistry()
	require.NoError(t, r.Register(toolpkg.Tool{
		Name:           "short",
		Description:    "Returns a short string",
		MaxBytes:       100,
		TruncationHint: "...truncated (total ${N} bytes)",
...
func TestHandler_BudgetExceeded(t *testing.T) {
	r := toolpkg.NewRegistry()
	require.NoError(t, r.Register(toolpkg.Tool{
		Name:           "long",
	...
	require.True(t, ok)
	assert.False(t, tr.IsError)
	assert.Less(t, len(tr.Content), 102)
	assert.Contains(t, tr.Content, "...truncated (total 102 bytes)")
}

func TestHandler_BudgetHintIncludesTotalBytes(t *testing.T) {
	r := toolpkg.NewRegistry()
	require.NoError(t, r.Register(toolpkg.Tool{
		Name:           "long",
		Description:    "Returns a long string",
	...
	tr, ok := tc.Turn.Artifacts[0].(artifact.ToolResult)
	require.True(t, ok)
	assert.True(t, strings.HasSuffix(tr.Content, "...truncated (total 102 bytes)"))
}

func TestHandler_BudgetUTF8Safe(t *testing.T) {
	r := toolpkg.NewRegistry()
	require.NoError(t, r.Register(toolpkg.Tool{
		Name:           "unicode",
		Description:    "Returns unicode content",
		MaxBytes:       22,
	...
	require.True(t, ok)
	assert.False(t, tr.IsError)
	assert.True(t, utf8.ValidString(tr.Content))
	assert.LessOrEqual(t, len(tr.Content), 22)
}

type mockRemoteSourceLarge struct {
	name string
}

func (m *mockRemoteSourceLarge) Name() string { return m.name }
func (m *mockRemoteSourceLarge) Tools() []toolpkg.Tool {
	return []toolpkg.Tool{
		{
			Name:           "long",
			Description:    "Returns a long string",
...
func (m *mockRemoteSourceLarge) Call(ctx context.Context, name string, args map[string]any) (any, error) {
	return strings.Repeat("a", 100), nil
}

func TestHandler_BudgetRemoteTool(t *testing.T) {
	remote := &mockRemoteSourceLarge{name: "remote"}
	r := toolpkg.NewRegistry(toolpkg.WithMCPServer(remote))
	h := NewHandler(r)
	emitter := &mockEmitter{}
