...
func TestInterfaces_TurnRunner_CallsThrough(t *testing.T) {
	...
	assert.Equal(t, ledger.RoleAssistant, st.Turns()[1].Role)
}
...
func TestInterfaces_TurnSubmitter_CallsThrough(t *testing.T) {
	...
	st, err := submitter.Submit(context.Background(), mem, ledger.RoleUser, artifact.Text{Content: "world"})
	require.NoError(t, err)
	...
	assert.Equal(t, st2.Turns()[0].Role, st.Turns()[0].Role)
	assert.Equal(t, st2.Turns()[1].Role, st.Turns()[1].Role)
	assert.Equal(t, ledger.RoleUser, st.Turns()[1].Role)
}
...
func TestInterfaces_TurnExecutor_CallsThrough(t *testing.T) {
	...
	st, err = executor.Submit(context.Background(), st, ledger.RoleUser, artifact.Text{Content: "user"})
	require.NoError(t, err)
