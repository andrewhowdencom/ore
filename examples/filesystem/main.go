...
func run() error {
	...
	if err := registry.Register(filesystem.ReadFileTool.Name, filesystem.ReadFileTool.Description, filesystem.ReadFileTool.Schema, filesystem.ReadFile, tool.WithDisplay(filesystem.ReadFileTool.DisplayHint)); err != nil {
		return fmt.Errorf("register read_file tool: %w", err)
	}
	if err := registry.Register(filesystem.WriteFileTool.Name, filesystem.WriteFileTool.Description, filesystem.WriteFileTool.Schema, filesystem.WriteFile, tool.WithDisplay(filesystem.WriteFileTool.DisplayHint)); err != nil {
		return fmt.Errorf("register write_file tool: %w", err)
	}
	if err := registry.Register(filesystem.EditFileTool.Name, filesystem.EditFileTool.Description, filesystem.EditFileTool.Schema, filesystem.EditFile, tool.WithDisplay(filesystem.EditFileTool.DisplayHint)); err != nil {
		return fmt.Errorf("register edit_file tool: %w", err)
	}
	if err := registry.Register(filesystem.ListDirectoryTool.Name, filesystem.ListDirectoryTool.Description, filesystem.ListDirectoryTool.Schema, filesystem.ListDirectory, tool.WithDisplay(filesystem.ListDirectoryTool.DisplayHint)); err != nil {
		return fmt.Errorf("register list_directory tool: %w", err)
	}
	if err := registry.Register(filesystem.SearchFilesTool.Name, filesystem.SearchFilesTool.Description, filesystem.SearchFilesTool.Schema, filesystem.SearchFiles, tool.WithDisplay(filesystem.SearchFilesTool.DisplayHint)); err != nil {
